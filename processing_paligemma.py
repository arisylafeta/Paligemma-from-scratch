...
def add_image_tokens_to_prompt(prefix_prompt, bos_token, image_seq_len, iamge_token):
...
def resize(
    ...
    height, width = size
    image = image.resize((width, height), resample=resample, reducing_gap=reducing_gap)
    return image

def rescale(image: np.ndarray, scale: float, dtype: np.dtype = np.float32) -> np.ndarray:
    rescaled_image = (image * scale).astype(dtype)
...
def process_images(
    ...
    images = [rescale(image, scale=rescale_factor) for image in images] # [0, 255] -> [0, 1.0]
